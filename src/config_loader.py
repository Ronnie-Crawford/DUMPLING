...
def format_config(config: dict) -> dict:
    ...
    # For predicted features we check which are available on the datasets chosen, and cross-check against those chosen in the config
    available_predicted_features = set()

    for dataset_name, label_name in config["SUBSETS_IN_USE"]:

        for feature in config["DATA"]["DATASETS"][dataset_name]["PREDICTED_FEATURE_COLUMNS"].keys():

            available_predicted_features.add(feature)

    desired_predicted_features = [key for key, value in config["DATA"]["PREDICTED_FEATURES"].items() if value]
    config["PREDICTED_FEATURES_LIST"] = [predicted_feature for predicted_feature in available_predicted_features if predicted_feature in desired_predicted_features]
    ...
    assert len(config["PREDICTED_FEATURES_LIST"]) > 0, "Must have non-zero amount of predicted features. Check the datasets chosen contain the features chosen."
